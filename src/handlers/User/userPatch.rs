...
#[patch("user/{id}")]
pub async fn patchUser(data: web::Data<AppState>, info: web::Json<UserOptional>, auth: BearerAuth, path: web::Path<String>) -> HttpResponse {
   ...
   match getPermissionLevelHttp(token, &data.pool).await {
      ...
      Ok(userPermissionLevel) => {
         if userPermissionLevel != PermissionLevel::Admin{
            ...
            if countOfAdded == 0{
               let errorPickUp: PickUpError = PickUpError::newMessage(PickUpErrorCode::BadRequest, "No fields provided");
