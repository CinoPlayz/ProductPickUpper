...
CREATE TABLE `User`(
    ...
    FOREIGN KEY (`FK_UserRole`) REFERENCES `UserRole`(`Id`) ON DELETE CASCADE,
	UNIQUE (`Username`)
) ENGINE=InnoDB;
...
CREATE TABLE `Token`(
	...
	`DeviceInfo` VARCHAR(128) NOT NULL,	
...
CREATE TABLE `Sales`(
	...
	`FK_User` VARCHAR(36) NOT NULL,
	FOREIGN KEY (`FK_Product`) REFERENCES `Product`(`Id`) ON DELETE CASCADE,
	FOREIGN KEY (`FK_Customer`) REFERENCES `Customer`(`Id`) ON DELETE CASCADE,
	FOREIGN KEY (`FK_User`) REFERENCES `User`(`Id`) ON DELETE CASCADE
) ENGINE=InnoDB;
...
/*ZipCodes for Slovenia (Date: 19.1.2024)*/
